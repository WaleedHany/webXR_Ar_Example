...
class App{
    ...
    initReticle() {
        ...
        const reticle = new THREE.Mesh(
            // to merge multiple buffer geometries
    ...
    {
        // get values of window bounds
        const width = window.innerWidth;
        const height = window.innerHeight;

        const vec = this.workingVec3; // Three.Vector3
        // copy the point passed as parameter
        vec.copy(point);
        // project this point to camera
        // this converts world space to Normalized Device Coordinates (NDC)
        vec.project(camera);
        // To convert to screen space width, 
        // we want to convert the range -1 to 1 to the range 0 to window.innerWidth
        // To convert to screen space hight, 
        // we want to convert the range -1 to 1 to the range window.innerHeight to 0
    ...
    setupXR(){
        ...
        function onSelect() {
            if (self.reticle.visible){
                ...
                // get position of reticle
                pt.setFromMatrixPosition(self.reticle.matrix)
                // add it to measurements array
                self.measurements.push(pt)

                if (self.measurements.length == 2) {
                    // Step 2 - we have a completed line so get its length, 
                    // create a label and reset the measurements array and currentLine
                    const distance = Math.round(self.getDistance(self.measurements) * 100);
                    // in oeder to show labels in ar, 
                    // we dynamically show a div object
                    const text = document.createElement('div');
                    text.className = 'label';
                    text.style.color = 'rgb(255,255,255)';
                    text.textContent = distance + ' cm';
                    document.querySelector('#container').appendChild(text);
                    // get center point of measuring line
                    self.labels.push(
                        { 
                            div: text, 
    ...
    render( timestamp, frame ) {
        ...
        this.labels.forEach( label => {
            const pos = self.toScreenPosition(
                label.point, self.renderer.xr.getCamera(self.camera));
